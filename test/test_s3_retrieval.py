...
class TestS3Retrieval:
    ...
    def fake_s3fs(self, mocker):
        ...
        mocker.patch("dclimate_zarr_client.s3_retrieval.get_s3_fs",
                     return_value=fake_s3fs)
        return fake_s3fs
    ...
    class TestGetDataSetFromS3Function:
        def test__given_a_dataset_name_and_bucker_name__it_fetch_the_dataset(self, mocker,
            ...
            s3Map_mock.assert_called_with(f"s3://{bucket_name}/datasets/{dataset_name}.zarr",
                                          s3=fake_s3fs, check=True)

    class TestListS3DatasetsFunction:
        def test__given_a_bucket_name__then__returns_all_zarr_datasets(self, mocker, fake_s3fs):
            bucket_name = "zarr-prod"
            datasets = ['zarr-dev/datasets/chirps_final_05-daily.zarr',
            ...
            assert s3_datasets == ['chirps_final_05-daily', 'chirps_prelim_05-daily',
                                   'copernicus_ocean_salinity_1p5_meters-daily',
                                   'cpc_precip_global-daily', 'cpc_precip_us-daily',
                                   'cpc_temp_max-daily', 'cpc_temp_min-daily',
                                   'era5_precip-hourly']
